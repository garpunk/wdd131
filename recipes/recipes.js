...
function getRandomListEntry(list) {
  if (!list || list.length === 0) {
    console.error('The list is empty or undefined.');
    return null;
  }
  return list[random(list.length)];
}

// Function to generate star rating with ARIA attributes
function generateStars(rating) {
  const fullStars = Math.floor(rating); // Number of full stars
  const halfStar = rating % 1 >= 0.5 ? 1 : 0; // Half star if applicable
  const emptyStars = 5 - fullStars - halfStar; // Remaining empty stars

  // Generate the ARIA label (e.g., "Rating: 4.5 out of 5 stars")
  const ariaLabel = `Rating: ${rating} out of 5 stars`;

  return `
    <span class="rating" role="img" aria-label="${ariaLabel}">
      ${'⭐'
        .repeat(fullStars)
        .split('')
        .map(
          (star) => `<span aria-hidden="true" class="icon-star">${star}</span>`
        )
        .join('')}
      ${
        halfStar
          ? `<span aria-hidden="true" class="icon-star-half">⭐</span>`
          : ''
      }
      ${'☆'
        .repeat(emptyStars)
        ...
        .map(
          (star) =>
            `<span aria-hidden="true" class="icon-star-empty">${star}</span>`
        )
        .join('')}
    </span>
  `;
...
function displayRecipe(recipe) {
    ...
    return `
      <span class="rating" role="img" aria-label="${ariaLabel}">
        ${'⭐'
          ...
          .map(
            (star) =>
              `<span aria-hidden="true" class="icon-star">${star}</span>`
          )
          .join('')}
        ${
          halfStar
  ...
  recipeDisplay.innerHTML = `
    ...
    recipe.name || 'No image'
      ...
      <p class="recipe-tag">${recipe.tags?.[0] || 'No tag available'}</p>
   
     <h2>${recipe.name || 'Unknown Recipe'}</h2>
    ${
      recipe.rating
...
// Call it with a random recipe
const randomRecipe = getRandomListEntry(recipes);
console.log(randomRecipe); // Debugging: Check if recipe is valid
